...
int FastaSlice(std::string query, std::string target, int slicesize, int leaps) {
    ...
    sprintf(command,
            "blastn -db %s -query /media/lab/Dee/chrom.map/query_v02.fsa  -outfmt '6 qseqid sstart sseqid length pident score evalue' -max_target_seqs 1 -out /media/lab/Dee/chrom.map/results_v03.out",
            target_database);
